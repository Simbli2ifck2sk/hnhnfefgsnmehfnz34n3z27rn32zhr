...
local SCRIPT_URL = "https://raw.githubusercontent.com/<user>/<repo>/main/MAINRobberWORKING.lua"
...
-- Server Hop Funktion (öffentliche Server)
local function hopToRandomServer()
    ...
    if not success then
        sendNotification("Fehler", "Konnte Server nicht laden - Reload...")
    ...
    if not success2 or not data or not data.data then
        sendNotification("Fehler", "Ungültige Server-Daten - Reload...")
        TeleportService:Teleport(game.PlaceId, player)
        return false
    end
    
    -- Verfügbare Server filtern (nicht voll, nicht der aktuelle)
    ...
    if #availableServers == 0 then
        sendNotification("Info", "Keine anderen Server verfügbar - Reload...")
        TeleportService:Teleport(game.PlaceId, player)
        return false
    end
    ...
    sendNotification("Server Hop", "Joine neuen Server...")
...
local function lootVisibleMeshParts(folder)
    ...
    if isPoliceNearby() then return end
...
-- Hauptfunktion: Bank rauben + Server Hop Logik (ENDLOS)
local function robBankAndHop()
    ...
    if bankLight2 and bankLight and bankLight2.Enabled == false and bankLight.Enabled == true then
        ...
        sendNotification("Bank Raub abgeschlossen", "Server Hop in 5 Sekunden...")
        
        -- NACH ERFOLGREICHEM RAUB: SERVER HOP
        task.wait(5)
        hopToRandomServer()
        
    else
        -- BANK IST GESCHLOSSEN - SOFORT SERVER HOP
        sendNotification("Bank geschlossen", "Server Hop in 5 Sekunden...")
        task.wait(5)
        hopToRandomServer()
    end
end

-- ENDLOSSCHLEIFE - nach Server Hop wird automatisch neu gestartet
-- weil das Script durch queue_on_teleport im neuen Server neu lädt
...
-- AUTO-START (nur einmal - der Rest passiert durch queue_on_teleport)
task.wait(3)
sendNotification("Bank Farm gestartet", "Endlosschleife - Bank -> Hop -> Bank...")
robBankAndHop() -- Einmal starten, dann übernimmt queue_on_teleport

-- Falls queue_on_teleport nicht funktioniert, hier eine Fallback-Logik:
-- Aber eigentlich sollte queue_on_teleport das übernehmen
