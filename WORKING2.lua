print("Bank Green/Red Monitor - Script Loaded")
...
-- Bank Status Check Function
local function checkBankStatus()
    local bankLight = nil
    local bankLight2 = nil
    
    pcall(function()
        local bankRobbery = Workspace:FindFirstChild("Robberies")
        if bankRobbery then
            local bankRobberyFolder = bankRobbery:FindFirstChild("BankRobbery")
            if bankRobberyFolder then
                bankLight = bankRobberyFolder:FindFirstChild("LightGreen")
                if bankLight then
                    bankLight = bankLight:FindFirstChild("Light")
                end
                bankLight2 = bankRobberyFolder:FindFirstChild("LightRed")
                if bankLight2 then
                    bankLight2 = bankLight2:FindFirstChild("Light")
                end
            end
        end
    end)
    
    if bankLight2 and bankLight then
        if bankLight2.Enabled == false and bankLight.Enabled == true then
            print("🟢 [BANK] OPEN - Ready to rob!")
            return "green"
        else
            print("🔴 [BANK] CLOSED - Waiting for reset")
            return "red"
        end
    else
        print("⚠️ [BANK] STATUS UNKNOWN - Cannot detect lights")
        return "unknown"
    end
end

-- Continuous Bank Status Monitor
local function startBankStatusMonitor()
    local lastStatus = nil
    
    task.spawn(function()
        while true do
            local status = checkBankStatus()
            
            -- Send notification on status change
            if status == "green" and lastStatus ~= "green" then
                sendNotification("Bank Status", "🟢 Bank is OPEN! Ready to rob!")
                print("🟢🟢🟢 BANK STATUS CHANGED TO OPEN 🟢🟢🟢")
            elseif status == "red" and lastStatus ~= "red" then
                sendNotification("Bank Status", "🔴 Bank is CLOSED! Waiting for reset...")
                print("🔴🔴🔴 BANK STATUS CHANGED TO CLOSED 🔴🔴🔴")
            end
...
local function hopToRandomServer()
    ...
    print("🌐 Attempting to hop to new server...")
    ...
    if #availableServers == 0 then
        ...
        print("❌ No servers with empty slots")
        return false
    end
    ...
    print("🌐 Hopping to server: " .. randomServer.serverName)
...
local function robBank()
    print("🏦 Starting bank robbery sequence...")
    ...
    if teamName == "Prisoner" then
        ...
        print("⛓️ Player is imprisoned, waiting for release...")
        repeat
            task.wait(5)
            team = player.Team
            teamName = team and team.Name or "None"
        until teamName ~= "Prisoner"
        print("✅ Player released from prison")
    end
    ...
    -- Prüfe Bank-Status mit Green/Red System
    local bankStatus = checkBankStatus()
    
    -- Bank Überfall nur wenn grün
    if bankStatus == "green" then
        print("🟢🟢🟢 BANK IS GREEN - PROCEEDING WITH ROBBERY 🟢🟢🟢")
        clickAtCoordinates(0.5, 0.9)
        ...
        if not hasGrenade() then
            print("💣 No grenade found, purchasing from dealer...")
            ensurePlayerInVehicle()
            MoveToDealer()
            task.wait(0.5)
        ...
        -- Granate werfen
        print("💣 Preparing to throw grenade...")
        ...
        for i, position in ipairs(Locations.bankCollectPositions) do
            print("📦 Collecting at position " .. i .. "/" .. #Locations.bankCollectPositions)
        ...
        print("✅✅✅ BANK ROBBERY COMPLETED SUCCESSFULLY ✅✅✅")
        sendNotification("Bank Raub abgeschlossen", "Nächster Check in 5 Minuten")
        
    elseif bankStatus == "red" then
        print("🔴🔴🔴 BANK IS RED (CLOSED) - SKIPPING ROBBERY 🔴🔴🔴")
        sendNotification("Bank geschlossen", "Warte 5 Minuten und versuche neuen Server")
    else
        print("⚠️⚠️⚠️ BANK STATUS UNKNOWN - SKIPPING ROBBERY ⚠️⚠️⚠️")
        sendNotification("Bank Status unbekannt", "Überspringe Raub")
    end
end
    ...
    task.spawn(function()
        while State.autofarmRunning do
            ...
            while waitTime > 0 and State.autofarmRunning do
                if waitTime % 30 == 0 then -- Print every 30 seconds
                    print("⏰ " .. math.floor(waitTime/60) .. " minutes " .. (waitTime % 60) .. " seconds remaining...")
                end
                task.wait(1)
                waitTime = waitTime - 1
            end
            ...
            if State.autofarmRunning then
                print("🌐 5 minutes elapsed - initiating server hop...")
...
-- Start bank status monitor
print("🟢🔴 Starting Bank Status Monitor...")
startBankStatusMonitor()

print("🚀 Starting Bank Autofarm...")
startAutofarm()

-- Mit "R" Taste manuell neu starten
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.R then
        print("🔘 Manual restart triggered with R key")
        if State.autofarmRunning then
            State.autofarmRunning = false
            task.wait(1)
        end
...
print("✅✅✅ Bank Robbery Script Fully Loaded ✅✅✅")
print("📊 Bank Status Monitor Active - Check console for 🟢/🔴 updates")
print("🔄 Press R to restart the autofarm")
